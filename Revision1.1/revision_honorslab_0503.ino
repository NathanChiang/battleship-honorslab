...
void setup() {
    ...
    while (count < 4) {
        ...
        int select = translateInput(button);
        int row = (select - 1) / 4;
        int col = (select - 1) % 4;
        matrix.fillRect(row*8 + 2, col*8 + 2, row*8 + 6, col*8 + 6, matrix.Color333(7, 7, 0));
        count++;
        if (player_board[select] == 1) {
            Serial.println("Position has already been chosen! Try again!");
        } else {
            player_board[select] = 1; //Changes value so that array is marked as ship at index
            count++;
        }
        reset(button);
        position = 17;
    ...
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            matrix.fillRect(i*8 + 2, j*8 + 2, i*8 + 6, j*8 + 6, matrix.Color333(7, 7, 7));
        }
    }
}
...
void loop() {
    ...
    */
    if (player_active) {
        for (int i = 0; i < sizeof(computer_board); i++) {
          int j = -1;
          if (i % 4 == 0) {
            j++;
          }
          if (computer_board[i] == 0 || 1) {
            matrix.fillRect(i*8 + 2, j*8 + 2, i*8 + 6, j*8 + 6, matrix.Color333(7, 7, 7));
          } else if (computer_board[i] == 2) {
            matrix.fillRect(i*8 + 2, j*8 + 2, i*8 + 6, j*8 + 6, matrix.Color333(0, 7, 0));
          } else if (computer_board[i] == 3) {
            matrix.fillRect(i*8 + 2, j*8 + 2, i*8 + 6, j*8 + 6, matrix.Color333(7, 0, 0));
          }
        }
        if (position <=  15 && position >= 0) { //checks if there was a valid turn
            if (computer_board[position] ==  0 || computer_board[position] ==  1) {
                if (computer_board[position] ==  1) { //player has hit a ship
                    ...
                    matrix.fillRect(row*8 + 2, col*8 + 2, row*8 + 6, col*8 + 6, matrix.Color333(0, 7, 0));
                    ...
                    int row = (position - 1) / 4;
                    int col = (position - 1) % 4;
                    matrix.fillRect(row*8 + 2, col*8 + 2, row*8 + 6, col*8 + 6, matrix.Color333(7, 0, 0));
                } else {
                    player_active = true;
    ...
    if (!player_active) { //computer only goes if player does not get another turn/try again
        ...
        for (int i = 0; i < sizeof(player_board); i++) {
          ...
          if (player_board[i] == 0) {
            matrix.fillRect(i*8 + 2, j*8 + 2, i*8 + 6, j*8 + 6, matrix.Color333(7, 7, 7));
          } else if (player_board[i] == 1) {
            matrix.fillRect(i*8 + 2, j*8 + 2, i*8 + 6, j*8 + 6, matrix.Color333(0, 7, 0));
          } else if (player_board[i] == 2) {
            matrix.fillRect(i*8 + 2, j*8 + 2, i*8 + 6, j*8 + 6, matrix.Color333(7, 0, 0));
          } else if (player_board[i] == 3) {
            matrix.fillRect(i*8 + 2, j*8 + 2, i*8 + 6, j*8 + 6, matrix.Color333(0, 0, 0));
          }
        }
        if (player_board[position] ==  1) { //computer has hit a ship
            ...
            int row = (position - 1) / 4;
            int col = (position - 1) % 4;
            matrix.fillRect(row*8 + 2, col*8 + 2, row*8 + 6, col*8 + 6, matrix.Color333(0, 0, 0));
        } else {
            player_active = true;
            Serial.print("There was an error!\n");
